...
public interface PostRepository extends JpaRepository<Post, String> {
    ...
    @Query(value = "SELECT new com.miniko.test.entities.post.PostViewDTO(u.name, u.avatar, p.file_link, p.description, TO_CHAR(p.date, 'DD/MM/YYYY às HH24:MI')) FROM posts p LEFT JOIN users u ON p.user_id = u.id ORDER BY p.date DESC")
